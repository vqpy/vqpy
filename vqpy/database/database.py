...
def _filter(obj: VObjBaseInterface,
            cond: Dict[str, Callable]) -> Optional[VObjBaseInterface]:
...
class VObjConstraint:
    ...
    def __add__(self, other: VObjConstraint) -> VObjConstraint:
        ...
        for key, cond in other.filter_cons.items():
            if key in ret.filter_cons:
                ret.filter_cons[key] = _cons_add(self.filter_cons[key], cond)
            else:
                ret.filter_cons[key] = cond
        return ret
    ...
    def apply(self, vobjs: List[VObjBaseInterface]) -> List[Dict]:
        ...
        filtered = list(map(lambda x: _filter(x, self.filter_cons), vobjs))
