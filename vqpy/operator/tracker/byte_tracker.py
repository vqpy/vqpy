...
class ByteTracker(GroundTrackerBase):
    ...
    class Data(BaseTrack):
        ...
        def update(self,
                   frame_id,
                   new_track: ByteTracker.Data,
                   reactivate=False,
                   newid=False):
            """Update a vobject, executed for all activated object per frame"""
        ...
        def tlbr_to_tlwh(tlbr):
            """Convert bounding box to format `(top left x, top left y, width,
            height)`.
            """
        ...
        def xyah_to_tlbr(xyah):
            """Convert  `(center x, center y, aspect ratio, height)` to
            bounding box format
            """
        ...
        # @jit(nopython=True)
        def xyah(self):
            """Bounding box in `(center x, center y, aspect ratio, height)`"""
    ...
    def _predict(self, track: Data):
        ...
        if track.state != TrackState.Tracked:
            mean_state[7] = 0
        prediction = track.kalman_filter.predict(mean_state, track.covariance)
        track.mean, track.covariance = prediction

    def _multipredict(self, tracks: List[Data]):
        if len(tracks) > 0:
            multi_mean = np.asarray([track.mean.copy()
                                     for track in tracks])
            multi_covariance = np.asarray([track.covariance
                                           for track in tracks])
            for i, track in enumerate(tracks):
                if track.state != TrackState.Tracked:
                    multi_mean[i][7] = 0
            prediction = self.shared_kalman.multi_predict(multi_mean,
                                                          multi_covariance)
            multi_mean, multi_covariance = prediction
    ...
    def _update(self,
        ...
        prediction = track.kalman_filter.update(track.mean,
                                                track.covariance,
                                                track.xyah)
        track.mean, track.covariance = prediction
    ...
    def update(self, data: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        ...
        result = matching.linear_assignment(dists, thresh=self.match_thresh)
        matches, u_track, u_detection = result
        ...
        result = matching.linear_assignment(dists, thresh=0.5)
        matches, u_track, u_detection_low = result
        for itracked, idet in matches:
        ...
        result = matching.linear_assignment(dists, thresh=0.7)
        matches, u_unconfirmed, u_detection = result
        for itracked, idet in matches:
        ...
        tracked = [t for t in self.tracked_stracks
                   if t.state == TrackState.Tracked]
        tracked = joint_stracks(tracked, activated_stracks)
        tracked = joint_stracks(tracked, refind_stracks)
        lost = sub_stracks(self.lost_stracks, tracked)
        lost.extend(lost_stracks)
        lost = sub_stracks(lost, self.removed_stracks)
        self.removed_stracks.extend(removed_stracks)
        tracked, lost = remove_duplicate_stracks(tracked, lost)
        self.tracked_stracks, self.lost_stracks = tracked, lost
...
def joint_stracks(tlista: List[ByteTracker.Data],
                  tlistb: List[ByteTracker.Data]) -> List[ByteTracker.Data]:
    exists = {}
    res = []
    for t in tlista:
        exists[t.track_id] = 1
...
def sub_stracks(tlista: List[ByteTracker.Data],
                tlistb: List[ByteTracker.Data]) -> List[ByteTracker.Data]:
    stracks = {}
    for t in tlista:
...
def remove_duplicate_stracks(stracksa: List[ByteTracker.Data],
                             stracksb: List[ByteTracker.Data]
                             ) -> Tuple[List[ByteTracker.Data],
                                        List[ByteTracker.Data]]:
